...
var tObjectCheck,
    ...
    oCONFIG = {
        ...
        SEARCH_PARAMS:
            'q|query|nasaInclude|k|querytext|keys|qt|search_input|search|globalSearch|goog|s|gsearch|search_keywords|SearchableText|sp_q|qs|psnetsearch|locate|lookup|search_api_views_fulltext|keywords|request|_3_keywords|searchString',
        YOUTUBE: !1,
...
function _defineCookieDomain() {
    /(([^.\/]+\.[^.\/]{2,3}\.[^.\/]{2})|(([^.\/]+\.)[^.\/]{2,4}))(\/.*)?$/.test(
        ...
        ? ((oCONFIG.COOKIE_DOMAIN = oCONFIG.SUBDOMAIN_BASED.toLowerCase().replace(/^www\./i, '')),
        ...
        ? ((oCONFIG.COOKIE_DOMAIN = document.location.hostname.match(
          ...
          (oCONFIG.SUBDOMAIN_BASED = !0))
        : ((oCONFIG.COOKIE_DOMAIN = location.hostname.toLowerCase().replace(/^www\./i, '')),
        ...
        switch (
            ...
            case 'pua':
                for (var c = _value.split(','), d = 0; d < c.length; d++)
                    _isValidGA4Num(c[d]) &&
                        (oCONFIG.GWT_GA4ID.push(c[d].toUpperCase()),
                        (oCONFIG.USING_PARALLEL_TRACKER = 'pua'));
                break;
            case 'pga4':
                for (var c = _value.split(','), d = 0; d < c.length; d++)
                    _isValidGA4Num(c[d]) &&
                        (oCONFIG.GWT_GA4ID.push(c[d].toUpperCase()),
                        (oCONFIG.USING_PARALLEL_TRACKER = 'pga4'));
                break;
            case 'agency':
        ...
        if ('page_view' === a.toLowerCase())
            try {
                if (Object.keys(b).length !== 0) {
                    ...
                    _sendEvent('page_view', b),
                        isSearch ? _sendViewSearchResult({ search_term: isSearch }) : '';
                }
            } catch (n) {}
        else
            try {
                var e_n = /((email|telephone|image|cta|navigation|faq|accordion)_)?click|file_download|view_search_results|video_(start|pause|progress|complete|play)|official_USA_site_banner_click|form_(start|submit|progress)|content_view|social_share|error|sort|filter|was_this_helpful_submit/gi.test(
                    a
                )
                    ? a
                    : 'dap_event';
...
function createTracker(a) {
    ...
    try {
        m =
            oCONFIG.USE_CUSTOM_URL && o.test(custom_dap_data.url)
                ? location.protocol +
                  '//' +
                  location.hostname +
                  custom_dap_data.url.replace(location.protocol + '//' + location.hostname, '')
    ...
    for (var b = 0; b < oCONFIG.GWT_GA4ID.length; b++) {
        if (b === 0) {
            ...
            document.referrer && -1 !== document.referrer.search(location.hostname)
                ? (r.page_referrer = _scrubbedURL(document.referrer))
                : document.referrer;
...
function _initAutoTracker() {
    ...
    document.addEventListener
        ? document.addEventListener('mousedown', _eventHandler, false)
        : document.attachEvent && document.attachEvent('onmousedown', _eventHandler);
    document.addEventListener
        ? document.addEventListener('keydown', _eventHandler, false)
        : document.attachEvent && document.attachEvent('onkeydown', _eventHandler);
}
...
if (oCONFIG.YOUTUBE) {
    ...
    onPlayerError = function(event) {
        _sendEvent('video_error', {
            videotitle:
                event.target.playerInfo !== undefined
    ...
    onPlayerStateChange = function(event) {
        try {
            ...
            var cTime =
                playerArray[videoIndex].playerInfo !== undefined
                    ? Math.round(playerArray[videoIndex].playerInfo.currentTime)
                    : Math.round(playerArray[videoIndex].getCurrentTime());
            var vDuration =
                playerArray[videoIndex].playerInfo !== undefined
                    ? Math.round(playerArray[videoIndex].playerInfo.duration)
                    : Math.round(playerArray[videoIndex].getDuration());
            var p = {
                ...
                video_title:
                    playerArray[videoIndex].playerInfo !== undefined
                        ? playerArray[videoIndex].playerInfo.videoData.title
                        : playerArray[videoIndex].getVideoData().title,
                video_id:
                    playerArray[videoIndex].playerInfo !== undefined
                        ? playerArray[videoIndex].playerInfo.videoData.video_id
                        : playerArray[videoIndex].getVideoData().video_id,
                video_url:
                    playerArray[videoIndex].playerInfo !== undefined
                        ? playerArray[videoIndex].playerInfo.videoUrl
                        : playerArray[videoIndex].getVideoUrl(),
            };
                    ...
                    ytUtils.push([
                        ...
                        function(videx) {
                            ...
                            setInterval(function() {
                                ...
                                if (
                                    y.video_percent <= _buckets[_buckets.length - 1] &&
                                    cCi < _buckets.length
                                ) {
                                    if (
                                        y.video_percent >= _buckets[cCi].milestone &&
                                        !_buckets[cCi].triggered &&
                                        _buckets[videoIndex].id === videoIndex
                                    ) {
                                        _buckets[cCi].triggered = true;
                                        y.video_percent = _buckets[cCi].milestone;
                                        y.video_current_time = Math.round(
                                            (y.video_duration / _buckets.length) * (cCi + 1)
                                        );
    ...
    YTUrlHandler = function(t) {
        return (
            ...
            (stAdd = ''),
            (adFlag = !1),
            -1 == t.indexOf('https') && (t = t.replace('http', 'https')),
            -1 == t.indexOf('?') && (stAdd = '?flag=1'),
            -1 == t.indexOf('enablejsapi') && ((stAdd += '&enablejsapi=1'), (adFlag = !0)),
            -1 == t.indexOf('origin') &&
    ...
    if (!window._isRedacted) {
        ...
        try {
            ...
            window.navigator.sendBeacon = function() {
                if (
                    arguments &&
                    arguments[0].match(/google-analytics\.com.*v\=2\&/i) &&
                    arguments[0].match(new RegExp(ga4_props))
                ) {
...
function _piiRegexReset() {
    window.piiRegex = [
        ...
        {
            name: 'SSN',
            regex: /((full)?(([\-\_])?)?ssn\=)?(\d{3}([\s\.\-\+]|%20)\d{2}([\s\.\-\+]|%20)\d{4})([^\&\s\?\/]*)/gi,
        },
        {
            name: 'NAME',
            regex: /((first|last|middle|sur|f|l|user)([\-\_])?)?name\=([^\&\s\?\/]*)/gi,
        },
        {
            name: 'PASSWORD',
            regex: /(((confirm([\-\_])?)?password)|passwd|pwd)\=([^\&\s\?\/]*)/gi,
        },
        {
            name: 'ZIP',
...
function _piiRedactor(payload, type) {
    try {
        ...
        payload =
            'object' === typeof payload && /json|default/.test(type)
                ? (_flattenJSON(payload), (payload = _objToQuery(payload)))
                : payload;
        _piiRegexReset();
        ...
        for (var i = 0; i < _hitPayloadParts.length; i++) {
            ...
            if (
                (_param[0].match(new RegExp(checkParams)) != null || /query|json/gi.test(type)) &&
                _val.indexOf('?') > -1
            ) {
                var paramArray = _val
                    .split('?')
            ...
            if (type === 'json') {
                window.piiRegex.push(
                    {
                        name: 'DOB',
                        regex: /(((birth)?date|dob)\=)(19|20)\d\d([\s\.\/\-]|%20)(0?[1-9]|1[012])([\s\.\/\-]|%20)(0?[1-9]|[12][0-9]|3[01])([^\&\s\?\/]*)/gi,
                        format: 'YYYY-MM-DD',
                    },
                    {
                        name: 'DOB',
                        regex: /(((birth)?date|dob)\=)(19|20)\d\d([\s\.\/\-]|%20)(0?[1-9]|[12][0-9]|3[01])([\s\.\/\-]|%20)(0?[1-9]|1[012])([^\&\s\?\/]*)/gi,
                        format: 'YYYY-DD-MM',
                    },
                    {
                        name: 'DOB',
                        regex: /(((birth)?date|dob)\=)(0?[1-9]|[12][0-9]|3[01])([\s\.\/\-]|%20)(0?[1-9]|1[012])([\s\.\/\-]|%20)(19|20)\d\d([^\&\s\?\/]*)/gi,
                        format: 'DD-MM-YYYY',
                    },
                    {
                        name: 'DOB',
                        regex: /(((birth)?date|dob)\=)(0?[1-9]|1[012])([\s\.\/\-]|%20)(0?[1-9]|[12][0-9]|3[01])([\s\.\/\-]|%20)(19|20)\d\d([^\&\s\?\/]*)/gi,
                        format: 'MM-DD-YYYY',
                    }
                );
            } else if (
            ...
            if (
                (_param[0].match(new RegExp(checkParams)) != null &&
                    _param[0].match(new RegExp(UncheckParams)) != null) ||
                /query|json|default/gi.test(type)
            ) {
...
function _URIHandler(a) {
    ...
    b.test(a) &&
        ((a = a.replace(b, '$1query$3')), (isSearch = a.match(/([?&])(query\=)([^&#?]*)/i)[3]));
    return a;
}
...
function _scrubbedURL(z) {
    ...
    var n = new RegExp(
        ...
        o = (n.test(z)
            ? z
            : document.location.protocol + '//' + document.location.hostname + z
        ).toLowerCase(),
        a = o.split('?')[0],
        r =
            o.split('?').length > 1
...
function _setAllowedQS() {
    ...
    _allowedQuerystrings = queries.default
        .concat(queries[oCONFIG.AGENCY.toLowerCase()])
        .concat(oCONFIG.SEARCH_PARAMS.toLowerCase().split('|'));
}
