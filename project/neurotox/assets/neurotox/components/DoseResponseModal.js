...
import DoseResponseParallelPlot from './DoseResponseParallelPlot';
...
class SingleCurveBody extends React.Component {
    render() {
        ...
        // for two urls
        let url = getDoseResponsesUrl([this.props.readout_id], [this.props.casrn]);
...
SingleCurveBody.propTypes = {
    readout_id: PropTypes.number.isRequired,
    casrn: PropTypes.string.isRequired,
};

class ParrallelCurveBody extends React.Component {
    render() {
        let url = getDoseResponsesUrl([this.props.readout_id], [this.props.casrn]);
        // console.log('ParrallelCurveBody');

        return <DoseResponseParallelPlot url={url} cols={1} height={400} collapse={NO_COLLAPSE} />;
    }
}

ParrallelCurveBody.propTypes = {
    readout_id: PropTypes.array.isRequired,
    casrn: PropTypes.string.isRequired,
};
