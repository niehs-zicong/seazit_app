    ...
    _renderPlot(d, yrange) {
        ...
        d.groupKeys.map((gk) => {
            ...
            if (d.curvep.length > 0) {
                d.curvep.filter((r) => r.groupKey == gk).forEach((el) => {
            ...
            // add hill if exists
            if (d.hill.length > 0) {
                d.hill.filter((r) => r.groupKey == gk).forEach((el) => {
                    if (el.bmd === null || el.bmd <= 0) {
                        return;
                    }
                        ...
                        hill = function(x0) {
                            return (
                                ...
                                el.param_vmax *
                                    ...
                                    (Math.pow(el.param_k, el.param_n) + Math.pow(x0, el.param_n))
                            );
                        },
                    ...
                    data.push({
                        ...
                        showlegend: !gk,
                        line: {
                            color: this.getMarkerColor(gk, 'hill'),
                            width: 2,
                        },
                        opacity: 0.8,
                    });
                    if (this.props.collapse === NO_COLLAPSE) {
                        annotations.push(`Hill: ${printFloat(el.bmd)} µM`);
                    }
                });
            }
        });
