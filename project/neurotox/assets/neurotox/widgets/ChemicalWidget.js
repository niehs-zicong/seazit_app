...
class ChemicalWidget extends BaseWidget {
    ...
    */
    ...
    _renderChemList(state) {
        return (
            <div>
                <label>Select chemical list: </label>
                <div className="radio">
                    <label>
                        <input
                            type="radio"
                            name="chemList"
                            onChange={this.handleChemlistChange}
                            value={CHEMLIST_80}
                            checked={state.chemList === CHEMLIST_80}
                        />
                        NTP 80
                    </label>
                    ...
                    <label>
                        <input
                            ...
                            onChange={this.handleChemlistChange}
                            value={CHEMLIST_91}
                            checked={state.chemList === CHEMLIST_91}
                        />
                        NTP 91
                    </label>
                    ...
                    <label>
                        <input
                            ...
                            onChange={this.handleChemlistChange}
                            value={CHEMLIST_ALL}
                            checked={state.chemList === CHEMLIST_ALL}
                        />
                        All
                    </label>
                </div>
    ...
    _renderSelector(state) {
        ...
        if (state.chemicalFilterBy === CHEMFILTER_CHEMICIAL) {
            opts = _.chain(state.tbl_substances)
                .groupBy('chemical__casrn')
                .values()
                .map((r) => r[0])
                .filter(filterInChemlist)
                .sortBy('chemical__name')
                .sortBy('chemical__category_id')
                .map((r) => {
                    return {
                        category: r.chemical__category_id,
                        key: r.chemical__casrn,
                        label: `${r.chemical__name} (${r.chemical__casrn})`,
                    };
                })
                .groupBy('category')
                .value();
            ...
            <div className="clearfix">
                ...
                <p className="help-block">
                    <a
                        ...
                        href={URL_CHEMXLSX}
                    >
                        <i className="fa fa-file-excel-o" />&nbsp; View categories
                    </a>
